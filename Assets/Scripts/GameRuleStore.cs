...
{
    [SyncVar(hook = nameof(SetIsRecommendRule_Hook))] private bool isRecommendRule;
    ...
    public void OnRecommmedToggle(bool value)
    {
    ...
    public void OnConfirmEjectToggle(bool value)
    {
    ...
    {
        emergencyMeetings = (int)Mathd.Clamp(emergencyMeetings + (isPlus ? 1 : -1), 0, 9);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }

    [SyncVar(hook = nameof(SetEmergencyMeetingsCooldown_Hook))] private int emergencyMeetingsCooldown;
    [SerializeField] private Text emergencyMeetingsCooldownText;
    ...
    {
        emergencyMeetingsCooldown = (int)Mathd.Clamp(emergencyMeetingsCooldown + (isPlus ? 5 : -5), 0, 60);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        meetingsTime = (int)Mathd.Clamp(meetingsTime + (isPlus ? 5 : -5), 0, 120);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        voteTime = (int)Mathd.Clamp(voteTime + (isPlus ? 5 : -5), 0, 300);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }

    [SyncVar(hook = nameof(SetAnonymousVotes_Hook))] private bool anonymousVotes;
    [SerializeField] private Toggle anonymousVotesToggle;
    ...
    {
        moveSpeed = (float)Mathd.Clamp(moveSpeed + (isPlus ? 0.25f : -0.25f), 0.5f, 3f);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        crewSight = (float)Mathd.Clamp(crewSight + (isPlus ? 0.25f : -0.25f), 0.25f, 5f);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        imposterSight = (float)Mathd.Clamp(imposterSight + (isPlus ? 0.25f : -0.25f), 0.25f, 5f);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }

    [SyncVar(hook = nameof(SetKillCooldown_Hook))] private float killCooldown;
    [SerializeField] private Text killCooldownText;
    public void SetKillCooldown_Hook(float _, float value)
    {
        killCooldownText.text = string.Format("{0:0.00}x", value);
        UpdateGameRuleOverview();
    }
    public void OnChangeKilldown(bool isPlus)
    {
        killCooldown = (float)Mathd.Clamp(killCooldown + (isPlus ? 0.25f : -0.25f), 10f, 60f);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        killRange = (EKillRange)Mathd.Clamp((int)killRange + (isPlus ? 1 : -1), 0, 2);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        taskBarUpdates = (ETaskBarUpdates)Mathd.Clamp((int)taskBarUpdates + (isPlus ? 1 : -1), 0, 2);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        commonTask = (int)Mathd.Clamp(commonTask + (isPlus ? 1 : -1), 0, 2);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        complexTask = (int)Mathd.Clamp(commonTask + (isPlus ? 1 : -1), 0, 3);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }
    ...
    {
        simpleTask = (int)Mathd.Clamp(simpleTask + (isPlus ? 1 : -1), 0, 5);
        isRecommendRule = false;
        isRecommendRuleToggle.isOn = false;
    }

    [SyncVar(hook = nameof(SetImposterCount_Hook))]
    [SerializeField] private int imposterCount;
